...
- [x] 接入播放停止、自然结束、换歌和音频欠载；
- [x] 根据 `trace_id` 生成独立的播放汇总投影。
...
- [x] 根据飞书组件上限拆分使用分析和设备稳定性仪表盘；日期粒度待真实样本积累后继续校准。
...
- [x] 部署向导增加飞书配置、首次授权和自动创建 Base 步骤；
- [x] README 补充 CLI 安装、登录和验证流程；
- [ ] 完整回归测试后提交并通过 PR 合入主分支。

## 里程碑 F：固件遥测与完整用户行为

- [x] 在 MCP 音频代理增加媒体临时令牌保护的播放遥测端点；
- [x] 扩展媒体清单，传递链路 ID、试听/完整类型、双时长和遥测地址；
- [x] 增加本地播放投影、重建、检查命令及乱序/幂等测试；
- [x] 固件异步上报唤醒、聆听、ASR、助手回复和实际播放状态；
- [x] 将本地播放投影同步到飞书业务表，并增加会话投影；
- [x] 实现按会话查询、删除和本地保留期清理；
- [x] 增加多设备、固件版本和错误分析视图。

代码、61 个 MCP 自动化测试和 EchoEar 完整固件构建均已通过。当前电脑未检测到 USB 串口，因此烧录后的真实设备闭环（播放时长、自然结束、停止/切歌、欠载以及听感无退化）仍待设备连接后完成。
