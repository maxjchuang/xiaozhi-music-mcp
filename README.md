...
MCP 工具说明会要求模型在遇到不熟悉的故事、绘本或图书时先联网搜索。为了让不同模型都能稳定触发，也可以在小智角色提示词中加入：

> 用户明确要求联网查询、问题涉及近期信息，或者用户要求讲述一个你不了解的故事、绘本或图书时，先调用 `web_search`。如果只搜到简介，可以概述或改编讲述，但不要声称复述了完整原文。

搜索工具返回的回答指引会要求模型使用短句和明确标点。故事等长内容每轮最多讲述约 300 个汉字，询问用户后再继续，以避免单次 TTS 合成过长导致后半段静音。
