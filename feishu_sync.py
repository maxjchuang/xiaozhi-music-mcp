...
def event_fields(event: AnalyticsEvent) -> dict[str, Any]:
    ...
    fields = {
        ...
        "缓存命中": bool(payload.get("cache_hit")),
        "传输来源": str(payload.get("delivery_source", ""))[:100],
        "缓存ID": str(payload.get("cache_id", ""))[:200],
        "缓存字节": payload.get("size_bytes") if isinstance(payload.get("size_bytes"), (int, float)) else None,
        "结果": result,
...
EVENT_TABLE_FIELDS = (
    ...
    {"name": "缓存命中", "type": "checkbox"},
    {"name": "传输来源", "type": "text"},
    {"name": "缓存ID", "type": "text"},
    {"name": "缓存字节", "type": "number"},
    {"name": "结果", "type": "text"},
...
DASHBOARD_BLOCKS = (
    ...
    ("ring", "缓存命中率", {"table_name": "原始事件", "count_all": True, "group_by": [{"field_name": "缓存命中", "mode": "integrated"}], "filter": {"conjunction": "and", "conditions": [{"field_name": "事件类型", "operator": "is", "value": "music_cache_lookup"}]}}),
    ...
    ("statistics", "缓存播放次数", {"table_name": "音乐播放记录", "count_all": True, "filter": {"conjunction": "and", "conditions": [{"field_name": "缓存播放", "operator": "is", "value": True}]}}),
...
def playback_fields(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    fields = {
        ...
        "缓存播放": bool(row.get("cache_hit")),
        "传输来源": row.get("delivery_source", ""),
        "缓存ID": row.get("cache_id", ""),
